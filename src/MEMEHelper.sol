...
contract MEMEHelper is IMEMEHelper, AccessControl {
     ...
     * - 5 分钟超时保护
     */
    function addLiquidityV2(address token, uint256 bnbAmount, uint256 tokenAmount)
        external
        payable
        onlyRole(CORE_ROLE)
     ...
     */
    function estimateLPTokens(address token, uint256 bnbAmount, uint256 tokenAmount) external view returns (uint256) {
     ...
     * @return feeBNB 手续费
     */
    function calculateTokenAmountOutWithFee(uint256 bnbIn, IMEMECore.BondingCurveParams memory curve, uint256 feeRate)
        external
        pure
        override
     ...
     * @return feeBNB 手续费
     */
    function calculateBNBAmountOutWithFee(uint256 tokenIn, IMEMECore.BondingCurveParams memory curve, uint256 feeRate)
        external
        pure
        override
     ...
     */
    function calculateRequiredBNB(uint256 tokenOut, IMEMECore.BondingCurveParams memory curve)
        external
        pure
        override
     ...
     * 注意：乘以 1e18 是为了保留精度
     */
    function getPrice(IMEMECore.BondingCurveParams memory curve) external pure override returns (uint256) {
