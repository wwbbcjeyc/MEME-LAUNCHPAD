...
interface IPancakePair {
    ...
    function totalSupply() external view returns (uint256); // LP 代币总供应量
    function balanceOf(address owner) external view returns (uint256); // 查询某地址的 LP 代币余额
    ...
    function transfer(address to, uint256 value) external returns (bool); // 转账
    ...
    function nonces(address owner) external view returns (uint256); // 用户签名随机数，防止重放攻击
    ...
    function MINIMUM_LIQUIDITY() external pure returns (uint256); // 最小流动性常数（1000）
    ...
    function price0CumulativeLast() external view returns (uint256); // token0 的累计价格（用于预言机）
    function price1CumulativeLast() external view returns (uint256); // token1 的累计价格（用于预言机）
    function kLast() external view returns (uint256); // 上一次更新的乘积常数
