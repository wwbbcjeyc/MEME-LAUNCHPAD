...
contract FutureLaunchTest is Test {
    ...
    function testSellAlsoRespectsLaunchTime() public {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                logs[i].topics[0] == keccak256("TokenCreated(address,address,string,string,uint256,bytes32)")
                    || logs[i].topics[0]
                        == keccak256("TokenCreatedWithInitialBuy(address,address,uint256,uint256,uint256)")
            ) {
                tokenAddress = address(uint160(uint256(logs[i].topics[1])));
                break;
            }
